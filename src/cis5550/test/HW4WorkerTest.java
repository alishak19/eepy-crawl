...
public class HW4WorkerTest extends GenericTest {
    ...
    void runTests(Set<String> tests) throws Exception {
        ...
        if (tests.contains("putif"))
            try {
                startTest("putif", "PUT a value and then GET it back", 0);
                Socket s = openSocket(8001);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                String cell = "/data/hello/world/v1";
                ...
                req = "PUT /data/hello/world/v2?ifcolumn=v1&equals=" + data;
                out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                ...
                s = openSocket(8001);
                out = new PrintWriter(s.getOutputStream());
                req = "PUT /data/hello/world/v2?ifcolumn=v1&equals=somethingelse";
                out.print(req + " HTTP/1.1\r\nHost: localhost\r\n\r\n");
                out.flush();
                r = readAndCheckResponse(s, "response");
                if (r.statusCode != 200)
        ...
        if (tests.contains("versions"))
            try {
                startTest("versions", "PUT a value and then GET it back", 0);
                Socket s = openSocket(8001);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                ...
                if (!r.headers.containsKey("version")
                        || !r.headers.get("version").equals("1")) testFailed("version header not correct");
                s.close();
                ...
                if (!r.body().equals("OK"))
                    testFailed("The server did return a 200 status code to our " + req
                            ...
                            + dump(r.body));
                if (!r.headers.containsKey("version")
                        || !r.headers.get("version").equals("2")) testFailed("version header not correct");
                s.close();
                ...
                if (!r.body().equals(data2))
                    testFailed("The server did return a 200 status code to our " + req
                            + ", but it was supposed to return the string we had PUT in earlier (" + data2
                            + "), and it didn't. Here is what was in the body instead:\n\n" + dump(r.body));
                s.close();
        ...
        if (tests.contains("read-id"))
            try {
                ...
                Thread.sleep(20000);
                File f = new File("__worker");
                if (id == null) id = (new Scanner(new File("__worker" + File.separator + "id"))).nextLine();
        ...
        if (tests.contains("put"))
            try {
                ...
                String req =
                        "PUT /data/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8);
                out.print(req + " HTTP/1.1\r\nHost: localhost\r\nContent-Length: " + data.length() + "\r\n\r\n" + data);
                out.flush();
                Response r = readAndCheckResponse(s, "response");
                if (r.statusCode != 200)
        ...
        if (tests.contains("overwrite"))
            try {
                ...
                String data2 = data1;
                while (data2.equals(data1)) data2 = randomAlphaNum(10, 20);
        ...
        if (tests.contains("stress"))
            try {
                                        ...
                                        if (random(0, 1) == 0) {
                                            ...
                                            synchronized (ver) {
                                                if (ver.containsKey(row))
                                                    expectedAtLeastVersion =
                                                            ver.get(row).intValue();
                                            }
                                            out.print("GET /data/" + tabname + "/" + row
                                                    + "/col HTTP/1.1\r\nHost: localhost\r\n\r\n");
                                            out.flush();
                                            Response r = readAndCheckResponse(s, "response");
                                            if (expectedAtLeastVersion > 0) {
                                                if (r.statusCode != 200) {
                                                    ...
                                                    System.err.println("GET returned code " + r.statusCode
                                                            + " for row: " + row + ", body: " + new String(r.body));
                                                } else {
                                                    String val = new String(r.body);
                                                    String[] pcs = val.split("-");
                                                    int v = Integer.valueOf(pcs[0])
                                                            .intValue();
                                                    if (v < expectedAtLeastVersion) {
                                                        testsFailed.increment();
                                                        System.err.println(
                                                                "GET returned version " + v + " for row " + row
                                                                        + ", but that has been overwritten; we expected at least version "
                                                                        + expectedAtLeastVersion);
                                                    }
                                                    if (!dat.get(row + "-" + v).equals(val)) {
                                                        testsFailed.increment();
                                                        System.err.println("GET returned value " + val + " for row "
                                                                + row + ", but we expected version " + v + " to be "
                                                                + dat.get(row + "-" + v)
                                                                        .equals(val));
                                                    }
                                                }
                                            ...
                                            int nextVersion = 0;
                                            synchronized (ver) {
                                                while (lck.containsKey(row)) row = randomAlphaNum(1, 1);
                                            ...
                                            if (r.statusCode != 200) {
                                                testsFailed.increment();
                                                System.err.println("PUT returned code " + r.statusCode + " for row: "
                                                        + row + ", body: " + new String(r.body));
                                            }
                                            synchronized (ver) {
                ...
                if (numFail > 0)
                    testFailed(
                            "Looks like " + numFail + " of the " + testsTotal
                ...
                if (reqPerSec < 1000)
                    testFailed(
                            "Looks like your solution handled " + reqPerSec
        ...
        if (stressRan)
            System.out.println("\nYour throughput in the stress test was " + reqPerSec + " requests/sec, with "
                    + numFail + " failues");
        cleanup();
        closeOutputFile();
    }
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (int i = 0; i < args.length; i++)
            if (!args[i].equals("all")
                    && !args[i].equals("auto1")
                    && !args[i].equals("auto2")
                    && !args[i].equals("setup1")
                    && !args[i].equals("setup2")
                    && !args[i].equals("cleanup1")
                    && !args[i].equals("cleanup2")) tests.add(args[i]);

        HW4WorkerTest t = new HW4WorkerTest();
        t.setExitUponFailure(exitUponFailure);
        if (outputToFile) t.outputToFile();
        if (runSetup1) t.runSetup1();
        if (runSetup2) t.runSetup2();
