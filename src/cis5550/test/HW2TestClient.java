...
public class HW2TestClient extends GenericTest {
    ...
    void prompt() {
        ...
        System.out.println(
                "In another terminal window, please run 'java cis5550.test.HW2TestServer, and then hit Enter in this window to continue.");
        (new Scanner(System.in)).nextLine();
    }

    void runSetup() throws Exception {
        Path path = Paths.get("__test123");
        if (!Files.exists(path)) (new File("__test123")).mkdir();
    ...
    void runTests(Set<String> tests) throws Exception {
        ...
        if (tests.contains("static"))
            try {
                ...
                if (r.statusCode != 200)
                    testFailed(
                            "The server returned a " + r.statusCode
                ...
                if (!r.body().equals("Well done is better than well said."))
                    testFailed(
                            "The server returned a response, but it was not the Benjamin Franklin quote we expected. Here is what we received instead:\n\n"
                                    + dump(r.body)
                ...
                if (r.statusCode != 404)
                    testFailed("The server returned a " + r.statusCode
                            + " response, but we were expecting a 404 Not Found, because the file we requested should not exist. Here is what was in the body:\n\n"
                            + dump(r.body));
                s.close();
                testSucceeded();
            } catch (Exception e) {
        ...
        if (tests.contains("get"))
            try {
                ...
                if (!r.body().equals("Hello World"))
                    testFailed(
                            "The server returned a response, but it was not the 'Hello World' we expected. Here is what we received instead:\n\n"
                                    + dump(r.body)
                                    + "\nCheck whether you are sending two (!) CRLFs at the end of the headers (not just one, and not just a CR), and whether you need to flush the OutputStream.");
                s.close();
                testSucceeded();
        ...
        if (tests.contains("post"))
            try {
                ...
                if (!r.body().equals("req.body() returned: " + theBody))
                    testFailed(
                            "The server returned a response, but it was not the string we expected. We sent a request with '"
                                    + theBody + "' in the body, and here is what the server sent back:\n\n"
                                    + dump(r.body)
        ...
        if (tests.contains("puthdr"))
            try {
                ...
                if (r.statusCode != 200)
                    testFailed(
                            "The server returned a " + r.statusCode
                                    + " response, but we were expecting a 200 OK. Here is the entire response:\n\n"
                                    + dump(r.response)
                                    + "\nCheck your path-matching code to make sure that the PUT route for /puthdr is actually being triggered.");
                if (!r.body().equals("OK"))
                    testFailed(
                            "The server returned a response, but it was not the 'OK' string we expected. Here is the body the server sent back:\n\n"
                                    + dump(r.body)
                ...
                if (r.headers.get("x-bar") == null)
                    testFailed(
                            "The server does not appear to have returned the X-Bar header the application was trying to generate. Here is the entire response that the server sent back (including headers):\n\n"
                                    + dump(r.response)
                ...
                if (!r.headers.get("x-bar").equals(theVal))
                    testFailed(
                            "The server did return the X-Bar header, but it didn't contain what we expected. We had sent the X-Foo header to '"
                                    + theVal
                                    + "', and the application was supposed to echo back that entire value in the X-Bar header. Here is the entire response that the server sent back (including headers):\n\n"
                                    + dump(r.response)
        ...
        if (tests.contains("pathpar"))
            try {
                ...
                if (!r.body().equals("Val1: [" + val1 + "] Val2: [" + val2 + "]"))
                    testFailed(
                            "The server returned a response, but it was not the string string we expected. Here is the body the server sent back:\n\n"
                                    + dump(r.body)
                                    + "\nCheck whether you are parsing the headers correctly. Remember to convert everything to lower case!!");
                s.close();
                testSucceeded();
        ...
        if (tests.contains("error"))
            try {
                ...
                if (r.statusCode != 500)
                    testFailed(
                            "The server returned a " + r.statusCode
                                    + " response, but we were expecting a 500 Internal Server Error. Here is the entire response:\n\n"
                                    + dump(r.response)
        ...
        if (tests.contains("write"))
            try {
                ...
                if (!r.body().equals("Hello World!"))
                    testFailed(
                            "The server returned a response, but it was not the string 'Hello World!' we expected. Here is the body the server sent back:\n\n"
                                    + dump(r.body)
                                    + "\nCheck whether you are parsing the headers correctly. Remember to convert everything to lower case!!");
                if (r.headers.get("x-bar") == null)
                    testFailed(
        ...
        if (tests.contains("qparam"))
            try {
                startTest("qparam", "Query parameters in URL and body", 10);
                Socket s = openSocket(8080);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                String val1 = randomAlphaNum(5, 5),
                        val2 = randomAlphaNum(5, 5),
                        val3 = randomAlphaNum(5, 5),
                        val4 = randomAlphaNum(5, 5);
                out.print("GET /qparam?par1=" + val1 + "+X&par2=%22" + val2
                ...
                if (!r.body().equals(expectedResponse))
                    testFailed(
                            "The server returned a response, but it was not the string '" + expectedResponse
                                    + "' we expected. Here is the body the server sent back:\n\n" + dump(r.body)
        ...
        if (tests.contains("multihost"))
            try {
                startTest("multihost", "Multiple hosts", 0);
                Socket s = openSocket(8080);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                out.print("GET /hosttest"
                        + " HTTP/1.1\r\nHost: myhost.com:80\r\n\r\n");
                out.flush();
                Response r = readAndCheckResponse(s, "response");
                String expectedResponse = "Hello1";
                if (r.statusCode != 200)
                    testFailed(
                            "The server returned a " + r.statusCode
                                    + " response, but we were expecting a 200 OK. Here is the entire response:\n\n"
                ...
                if (r.statusCode != 404)
                    testFailed(
                            "The server returned a " + r.statusCode
                                    + " response, but we were expecting a 404 OK. Here is the entire response:\n\n"
                                    + dump(r.response)
                                    + "\nCheck your path-matching code to make sure that the GET route for /write is actually being triggered.");
                s.close();
                testSucceeded();
        ...
        if (tests.contains("redirect"))
            try {
                startTest("redirect", "Redirect", 0);
                Socket s = openSocket(8080);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                out.print("GET /redirect HTTP/1.1\r\nHost: localhost:8000\r\n\r\n");
                out.flush();
                Response r = readAndCheckResponse(s, "response");
                if (r.statusCode != 301)
                    testFailed(
        ...
        if (tests.contains("beforeafter1"))
            try {
                startTest("beforeafter1", "Test before after", 0);
                Socket s = openSocket(8080);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                out.print("GET /beforeafter1"
                        + " HTTP/1.1\r\nHost: beforeafter.com\r\n\r\n");
                out.flush();
                Response r = readAndCheckResponse(s, "response");
                String expectedResponse = "Hello BeforeAfter";
                if (r.statusCode != 200)
                    testFailed(
                            "The server returned a " + r.statusCode
                                    + " response, but we were expecting a 200 OK. Here is the entire response:\n\n"
                ...
                if (!r.body().equals(expectedResponse))
                    testFailed(
                            "The server returned a response, but it was not the string '" + expectedResponse
                                    + "' we expected. Here is the body the server sent back:\n\n" + dump(r.body));
                if (!r.headers.containsKey("x-before") || !r.headers.containsKey("x-after"))
                    testFailed(
                            "The server did not return the X-Before and X-After headers we expected. Here is the entire response that the server sent back (including headers):\n\n"
                                    + dump(r.response));
                s.close();
                testSucceeded();
            } catch (Exception e) {
        ...
        if (tests.contains("beforeafter2"))
            try {
                startTest("beforeafter2", "Test before after", 0);
                Socket s = openSocket(8080);
                PrintWriter out = new PrintWriter(s.getOutputStream());
                out.print("GET /beforeafter2"
                        + " HTTP/1.1\r\nHost: beforeafter.com\r\n\r\n");
                out.flush();
                Response r = readAndCheckResponse(s, "response");
                String expectedResponse = "Hello BeforeAfter";
        ...
        else System.out.println(numTestsFailed + " test(s) failed.");
    ...
    public static void main(String args[]) throws Exception {
        ...
        if ((args.length == 0) || args[0].equals("all") || args[0].equals("auto")) {
            ...
            tests.add("multihost");
            tests.add("redirect");
            tests.add("beforeafter1");
            tests.add("beforeafter2");
        }
        ...
        for (int i = 0; i < args.length; i++)
            if (!args[i].equals("all")
                    && !args[i].equals("auto")
                    && !args[i].equals("setup")
                    && !args[i].equals("cleanup")) tests.add(args[i]);

        HW2TestClient t = new HW2TestClient();
        t.setExitUponFailure(exitUponFailure);
        if (outputToFile) t.outputToFile();
        if (runSetup) t.runSetup();
        if (promptUser) t.prompt();
        if (runTests) t.runTests(tests);
        if (cleanup) t.cleanup();
    }
}
