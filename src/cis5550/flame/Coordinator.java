...
class Coordinator extends cis5550.generic.Coordinator {
    ...
    public static void main(String args[]) {
        ...
        post("/submit", (request, response) -> {
            ...
            // Extract the parameters from the query string. The 'class' parameter, which contains the main class, is
            // mandatory, and
            // we'll send a 400 Bad Request error if it isn't present. The 'arg1', 'arg2', ..., arguments contain
            // command-line
            ...
            // We begin by uploading the JAR to each of the workers. This should be done in parallel, so we'll use a
            // separate
            // thread for each upload.

            Thread threads[] = new Thread[getFlameWorkers().size()];
            String results[] = new String[getFlameWorkers().size()];
            for (int i = 0; i < getFlameWorkers().size(); i++) {
                final String url = "http://" + getFlameWorkers().elementAt(i) + "/useJAR";
                final int j = i;
                threads[i] = new Thread("JAR upload #" + (i + 1)) {
                    public void run() {
                        try {
                            results[j] = new String(HTTP.doRequest("POST", url, request.bodyAsBytes())
                                    .body());
                        } catch (Exception e) {
            ...
            // Write the JAR file to a local file. Remember, we will need to invoke the 'run' method of the job, but, if
            // the job
            // is submitted from a machine other than the coordinator, the coordinator won't have a local copy of the
            // JAR file. We'll use
            ...
            return myFlameContext.getOutput();
        });
    ...
    public static Vector<String> getFlameWorkers() {
