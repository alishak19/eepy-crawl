...
echo "cd '$(pwd)'; java -XX:ActiveProcessorCount=4 -cp bin cis5550.kvs.Coordinator 8000" > kvscoordinator.sh
...
do
    ...
    echo "cd '$(pwd)'; java -XX:ActiveProcessorCount=4 -cp bin cis5550.kvs.Worker $((8000+$i)) $dir localhost:8000" > kvsworker$i.sh
...
echo "cd '$(pwd)'; java -XX:ActiveProcessorCount=4 -cp bin cis5550.flame.Coordinator 9000 localhost:8000" > flamecoordinator.sh
...
do
    echo "cd '$(pwd)'; java -XX:ActiveProcessorCount=4 -cp bin cis5550.flame.Worker $((9000+$i)) localhost:9000" > flameworker$i.sh
